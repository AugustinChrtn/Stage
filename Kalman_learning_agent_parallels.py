...
class Kalman_agent_parallels(): 
    ...
    def __init__(self, environment, gamma=0.8, variance_ob=1,variance_tr=0.01,gamma_curiosity=0.5):
        ...
        self.KF_table_curiosity=dict() # Dictonaries to store mean, variance and a curiosity variance
    ...
    def learn(self, old_state, reward, new_state, action):
        ...
        self.KF_table_curiosity[old_state][action] = ((current_variance+self.variance_tr)*(self.KF_table_variance[old_state][action]+self.gamma_curiosity*max_curiosity_in_new_state)+(self.variance_ob*current_mean))/ (current_variance+self.variance_tr+self.variance_ob)
