...
class Kalman_agent_softmax(): 
    ...
    def __init__(self, environment, gamma=1, variance_ob=1,variance_tr=40,curiosity_factor=1.8,gamma_curiosity=0.5,alpha=1,beta=0.02):
        ...
        for x in range(environment.height):
            for y in range(environment.width):
                ...
                self.KF_table_variance[(x,y)] = {'UP':10, 'DOWN':1, 'LEFT':1, 'RIGHT':1}
    ...
    def choose_action(self):
        ...
        total,m=0,max(dict_probas.values())
        for value in dict_probas.values() :
            total+=np.exp((value-m)*self.beta)
        for key,value in dict_probas.items():
                dict_probas[key]=np.exp(self.beta*(value-m))/total
        action=random.choices(list(dict_probas.keys()),weights=dict_probas.values(),k=1)[0]
